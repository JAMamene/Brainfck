...
public class TestLevel2 {
    ...
    public void tests5_1() {
        String[] args = {"-p", PATH + "Test1"}; // short syntax
    ...
    public void tests5_2() {
        String[] args = {"-p", PATH + "Test1,5"}; // same as above but with mixed syntax
        perf = new CommandPerform(args); //will perform the actions needed
        perf.performAll();
        bfck = perf.getContainer().getBfck();
    ...
    public void tests6_1() {
        ...
        String[] args = {"-p", PATH + "Test1", "--rewrite"}; // rewrite code
        perf = new CommandPerform(args); //will perform the actions needed
    ...
    public void tests6_2() {
        ...
        baos = new ByteArrayOutputStream(); // will redirect output to string
        System.setOut(new PrintStream(baos));
        String[] args = {"-p", PATH + "Test1,5", "--rewrite"}; // same as above but with mixed syntax
        perf = new CommandPerform(args); //will perform the actions needed
        perf.performStoppingAction(); // no need for other commands
        assertEquals("++++++++>>>++<++--\r\n", baos.toString()); // assert code is the same in short syntax
    }
    ...
    public void tests7_1() {
        ...
        System.setOut(new PrintStream(baos));
        String[] args = {"-p", PATH + "Test1.bmp", "--rewrite"};
    ...
    public void tests7_2() {
        ...
        System.setOut(new PrintStream(baos));
        String[] args = {"-p", PATH + "Test1,5.bmp", "--rewrite"};
        perf = new CommandPerform(args);
        perf.performStoppingAction(); // no need for other commands
        assertEquals("++++++++>>>++<++--\r\n", baos.toString()); // The image in short syntax should be like this
    }
    ...
    public void tests8_1() {
        exit.expectSystemExit();
        String[] args = {"-p", PATH + "Test1,5", "--translate"}; // use translate option to create bmp from Test1,5
        perf = new CommandPerform(args); //will perform the actions needed
        perf.performStoppingAction(); // no need for other commands
        baos = new ByteArrayOutputStream(); // Now we use the image created to check if it is as expected (test7_2)
        System.setOut(new PrintStream(baos));
        String[] args2 = {"-p", PATH + "Test1,5.bmp", "--rewrite"};
    ...
    public void tests9_1() {
        String[] args = {"-p", PATH + "Test2"}; // mixed syntax with in and out in short syntax
    ...
    public void tests9_2() {
        String[] args = {"-p", PATH + "Test2,5"}; // mixed syntax with in and out in long syntax
        String str = "5";
    ...
    public void tests9_3() {
        String[] args = {"-p", PATH + "Test2", "-i", PATH + "ITestInput"}; // ITestInput contains 34;
        perf = new CommandPerform(args); //will perform the actions needed
        perf.performAll();
        bfck = perf.getContainer().getBfck();
        assertEquals(0 - MASK.get(), bfck.getMemoryAt((short) 0)); // assert if the memory state is as expected
    ...
    public void tests9_4() {
        ...
        System.setOut(new PrintStream(baos));
        String[] args = {"-p", PATH + "Test2", "--rewrite"};
        perf = new CommandPerform(args);
        perf.performStoppingAction(); // no need for other commands
        assertEquals(">>++++++++--<,.++\r\n", baos.toString());
    }
    ...
    public void tests10_1() {
        String[] args = {"-p", PATH + "TestCheck1", "--check"};
    ...
    public void tests10_2() {
        String[] args = {"-p", PATH + "TestCheck2", "--check"}; //too many closing brackets
    ...
    public void tests10_3() {
        String[] args = {"-p", PATH + "TestCheck3", "--check"}; //closing bracket at the beginning
        exception.expect(SyntaxException.class);
        exit.expectSystemExitWithStatus(4); // check should raise exception
        perf = new CommandPerform(args); // should terminate the program with exit code 4
        perf.performStoppingAction();
    ...
    public void tests10_4() {
        String[] args = {"-p", PATH + "TestCheck1", "--check", "--translate", "--rewrite"}; //closing bracket at the beginning
        exception.expect(SyntaxException.class);
    ...
    public void tests11_1() {
        String[] args = {"-p", PATH + "TestAddition"}; //3 + 4
        perf = new CommandPerform(args); //will perform the actions needed
        perf.performAll();
        bfck = perf.getContainer().getBfck();
    ...
    public void tests11_2() {
        String[] args = {"-p", PATH + "TestMultiplication"}; //8 * 7
        perf = new CommandPerform(args); //will perform the actions needed
        perf.performAll();
        bfck = perf.getContainer().getBfck();
